using CustomCharacter.Mechanics;
...
public partial class Player : AnimatedEntity
{
	...
	public override void Simulate( IClient cl )
	{
		...
		{
			var tr = Trace.Ray( EyePosition, EyePosition + EyeRotation.Forward * 1000f )
				.Ignore( this )
			...
			if ( tr.Hit )
			{
				foreach ( var npc in All.OfType<NpcTest>() )
				{
					if ( npc.IsValid() )
					{
						npc.Steer = new();
		...
		if ( Input.Pressed( "attack2" ) && Game.IsServer )
		{
			...
			if ( tr.Hit )
			{
				...
				npc.SetModel( "models/biped_standard/biped_standard.vmdl" );
			}
		}
